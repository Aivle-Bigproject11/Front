...
const Menu1_2 = () => {
    ...
    const getGroupIcon = (groupName) => ({
        상조회사정보: <Users size={20} />, 고객정보: <User size={20} />,
        고인기본정보: <FileText size={20} />, 신고인정보: <Phone size={20} />,
        제출인정보: <FileText size={20} />, 장례정보: <MapPin size={20} />
    ...
    return (
        <div className="page-wrapper" style={{
            ...
            <div className={`dashboard-container ${animateCard ? 'animate-in' : ''}`} style={{
                ...
                <div style={{ flex: '0 0 400px', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{
                        ...
                        <div style={{ width: '120px', height: '120px', background: 'rgba(111, 66, 193, 0.2)', borderRadius: '50%', margin: '0 auto 30px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 10px 30px rgba(0,0,0,0.2)' }}>
                        ...
                        <div style={{ marginTop: '30px', borderTop: '1px solid #eee', paddingTop: '20px'}}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', marginBottom: '12px' }}><Calendar size={16} style={{ color: '#6f42c1', marginRight: '10px' }} /><span style={{fontWeight: 500}}>별세일: {customerUtils.formatDate(selectedCustomer.funeralDate)}</span></div>
                            <div style={{ display: 'flex', alignItems: 'center' }}><MapPin size={16} style={{ color: '#6f42c1', marginRight: '10px' }} /><span style={{fontWeight: 500}}>주소 : {selectedCustomer.location}</span></div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="form-scroll-area" style={{ flex: 1, overflowY: 'auto', paddingRight: '10px' }}>
                        {Object.entries(formGroups).map(([groupName, fields]) => (
                            <Card key={groupName} className="mb-4">
                                <Card.Header><h5 style={{ margin: 0, display: 'flex', alignItems: 'center' }}>{getGroupIcon(groupName)}<span style={{ marginLeft: '8px' }}>{groupName}</span></h5></Card.Header>
                                <Card.Body>
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
                                        {fields.map((fieldName) => (
                                            <Form.Group key={fieldName}>
                                                <Form.Label>{fieldLabels[fieldName] || fieldName}{validationRules.required?.includes(fieldName) && <span style={{ color: '#dc3545', marginLeft: '4px' }}>*</span>}</Form.Label>
                                                <Form.Control
            ...
            <style jsx global>{`
                ...
                .form-control:focus { box-shadow: 0 0 0 3px rgba(102, 126, 234, 0.1) !important; border-color: #667eea !important; }
