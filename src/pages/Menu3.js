...
const Menu3 = () => {
    ...
    const getFamilyInfo = (customer) => `${customer.isMarried ? '기혼' : '미혼'}, ${customer.hasChildren ? '자녀 있음' : '자녀 없음'}`;
     ...
     const handleHistoryClick = async (customer) => {
        ...
        setMessageHistory([]); // 이전 기록을 먼저 비웁니다.

        try {
            //  가장 최신 기록만 가져오는 API를 호출
            const response = await recommendationService.getLatestCustomerHistory(customer.id);

            // API 응답이 유효한 객체인지 확인
            if (response.data && Object.keys(response.data).length > 0) {
                // 응답은 단일 객체이지만, .map()으로 렌더링하기 위해 배열로 감싸서 상태에 저장
                setMessageHistory([response.data]);
            } else {
                // 응답이 비어있으면(기록이 없으면) 빈 배열을 유지
                setMessageHistory([]);
            }
        } catch (err) {
                        ...
                        <div className="mt-3" style={{ flexShrink: 0 }}>
                           <Row>
                                <Col lg={6} className="mb-3 mb-lg-0">
                                    <Card style={{ background: 'rgba(253, 251, 243, 0.92)', border: '1px solid rgba(184, 134, 11, 0.2)', height: '100%' }}>
                                        <Card.Header as="h5" style={{color: '#2C1F14', background: 'rgba(184, 134, 11, 0.1)'}}>메시지 미리보기</Card.Header>
                                        <Card.Body className="d-flex flex-column">
                                            <Form.Control as="textarea" rows={8} value={messagePreview} onChange={(e) => setMessagePreview(e.target.value)} className="mb-3 flex-grow-1" style={{whiteSpace: 'pre-wrap'}} />
                                            <div className="d-flex justify-content-end gap-2">
                                                <Button variant="secondary" onClick={handleEditMessage}>메시지 수정</Button>
                                                <Button className="btn-golden" onClick={handleSendMessage}>메시지 전송</Button>
                                            </div>
                                        </Card.Body>
                                    </Card>
                                </Col>
                                <Col lg={6}>
                                    <Card style={{ background: 'rgba(253, 251, 243, 0.92)', border: '1px solid rgba(184, 134, 11, 0.2)', height: '100%' }}>
                                        <Card.Header as="h5" style={{color: '#2C1F14', background: 'rgba(184, 134, 11, 0.1)'}}>메시지 발송 기록</Card.Header>
                                                    ...
                                                    messageHistory.map(history => {
                                                        ...
                                                        return (
                                                            <div key={history.id} className="mb-4">
                                                                ...
                                                                <p style={{whiteSpace: 'pre-wrap', fontSize: '0.9rem'}}>{history.message}</p>
                                                                <hr/>
                                                            </div>
                                                        );
                                                    })
